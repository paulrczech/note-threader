# Note Threader — Claude Context

## What this app is
A minimal music utility for voice leading guided by oblique strategies. Users move individual voices by small intervals — the chord emerges as a byproduct. Inspired by Brian Eno's Oblique Strategies and Chopin's voice-movement approach. Target: songwriters/composers.
...
- `src/components/playback/PlaybackControls.vue` — loop/stop/play-once + octave +8/−8 buttons
...
```css
--font-serif: 'Cormorant Garamond', Georgia, serif  /* poetry, headings, strategy text */
--font-mono:  'SF Mono', 'Fira Code', monospace     /* note names, data */
--font-sans:  system-ui sans                         /* controls, body */
--color-bg:          #0d0d12
--color-surface:     #161620
--color-accent:      #e0a87c   /* amber */
--color-text:        #e4e4dc
--color-text-dim:    #8888a8   /* labels, secondary UI */
--color-border:      #2a2a38
...
## Copy/labels
- "let it begin" (random start), "begin here" (manual start)
- "keep this" / "keep these N" (confirm button)
- "another" (strategy redraw button)
- "+8" / "−8" (octave transpose)
