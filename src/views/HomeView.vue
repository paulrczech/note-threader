    ...
    <ion-content class="ion-padding" fullscreen>
      ...
      <div class="home-layout">
        ...
        <div class="title-block">
          <h1 class="app-title">note threader</h1>
          <p class="app-tagline">voice leading by oblique strategy</p>
        </div>
        ...
        <div class="param-group">
          <button class="ghost-btn" @click="showManual = !showManual">
            {{ showManual ? '▾' : '▸' }} enter notes manually
          </button>
          ...
          <div v-if="showManual" class="manual-entry">
            <div
              ...
              class="voice-picker"
            >
              <span class="voice-label" :style="{ color: voiceColors[i] }">V{{ i + 1 }}</span>
              <select
                v-model="manualMidi[i]"
                class="note-select"
              >
                <option
...
async function startManual() {
  ...
  if (!isValidCluster(sorted)) {
    manualError.value = 'Invalid cluster — check range and spread (max 14 semitones)'
    return
  }
...
.ghost-btn:hover { color: var(--color-text); }
